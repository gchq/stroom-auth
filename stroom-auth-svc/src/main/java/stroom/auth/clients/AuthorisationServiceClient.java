...
public class AuthorisationServiceClient {
    ...
    public List<User> getUsersGroups(String userId, String usersJws) {
        String authorisationUrl = config.getAuthorisationServiceConfig().getGroupsForUserUrl() + "/" + userId;
    ...
    public boolean isUserAuthorisedToManageUsers(String userId, String usersJws) {
        ...
        Optional<User> optionalGroup = groups.stream().filter(group ->
                group.getName().equals(config.getAuthorisationServiceConfig().getCanManageUsersPermission()))
                .findFirst();
