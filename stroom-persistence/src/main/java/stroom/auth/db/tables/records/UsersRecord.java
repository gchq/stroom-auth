/*
 * This file is generated by jOOQ.
 */
package stroom.auth.db.tables.records;


import java.sql.Timestamp;

import javax.annotation.Generated;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record18;
import org.jooq.Row18;
import org.jooq.impl.UpdatableRecordImpl;

import stroom.auth.db.tables.Users;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.11.9"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class UsersRecord extends UpdatableRecordImpl<UsersRecord> implements Record18<Integer, String, String, Timestamp, String, String, String, String, Integer, Integer, Timestamp, Timestamp, String, Timestamp, String, Boolean, Timestamp, Boolean> {

    private static final long serialVersionUID = -456050648;

    /**
     * Setter for <code>auth.users.id</code>.
     */
    public void setId(Integer value) {
        set(0, value);
    }

    /**
     * Getter for <code>auth.users.id</code>.
     */
    public Integer getId() {
        return (Integer) get(0);
    }

    /**
     * Setter for <code>auth.users.email</code>.
     */
    public void setEmail(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>auth.users.email</code>.
     */
    public String getEmail() {
        return (String) get(1);
    }

    /**
     * Setter for <code>auth.users.password_hash</code>.
     */
    public void setPasswordHash(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>auth.users.password_hash</code>.
     */
    public String getPasswordHash() {
        return (String) get(2);
    }

    /**
     * Setter for <code>auth.users.password_last_changed</code>.
     */
    public void setPasswordLastChanged(Timestamp value) {
        set(3, value);
    }

    /**
     * Getter for <code>auth.users.password_last_changed</code>.
     */
    public Timestamp getPasswordLastChanged() {
        return (Timestamp) get(3);
    }

    /**
     * Setter for <code>auth.users.state</code>.
     */
    public void setState(String value) {
        set(4, value);
    }

    /**
     * Getter for <code>auth.users.state</code>.
     */
    public String getState() {
        return (String) get(4);
    }

    /**
     * Setter for <code>auth.users.first_name</code>.
     */
    public void setFirstName(String value) {
        set(5, value);
    }

    /**
     * Getter for <code>auth.users.first_name</code>.
     */
    public String getFirstName() {
        return (String) get(5);
    }

    /**
     * Setter for <code>auth.users.last_name</code>.
     */
    public void setLastName(String value) {
        set(6, value);
    }

    /**
     * Getter for <code>auth.users.last_name</code>.
     */
    public String getLastName() {
        return (String) get(6);
    }

    /**
     * Setter for <code>auth.users.comments</code>.
     */
    public void setComments(String value) {
        set(7, value);
    }

    /**
     * Getter for <code>auth.users.comments</code>.
     */
    public String getComments() {
        return (String) get(7);
    }

    /**
     * Setter for <code>auth.users.login_failures</code>.
     */
    public void setLoginFailures(Integer value) {
        set(8, value);
    }

    /**
     * Getter for <code>auth.users.login_failures</code>.
     */
    public Integer getLoginFailures() {
        return (Integer) get(8);
    }

    /**
     * Setter for <code>auth.users.login_count</code>.
     */
    public void setLoginCount(Integer value) {
        set(9, value);
    }

    /**
     * Getter for <code>auth.users.login_count</code>.
     */
    public Integer getLoginCount() {
        return (Integer) get(9);
    }

    /**
     * Setter for <code>auth.users.last_login</code>.
     */
    public void setLastLogin(Timestamp value) {
        set(10, value);
    }

    /**
     * Getter for <code>auth.users.last_login</code>.
     */
    public Timestamp getLastLogin() {
        return (Timestamp) get(10);
    }

    /**
     * Setter for <code>auth.users.created_on</code>.
     */
    public void setCreatedOn(Timestamp value) {
        set(11, value);
    }

    /**
     * Getter for <code>auth.users.created_on</code>.
     */
    public Timestamp getCreatedOn() {
        return (Timestamp) get(11);
    }

    /**
     * Setter for <code>auth.users.created_by_user</code>.
     */
    public void setCreatedByUser(String value) {
        set(12, value);
    }

    /**
     * Getter for <code>auth.users.created_by_user</code>.
     */
    public String getCreatedByUser() {
        return (String) get(12);
    }

    /**
     * Setter for <code>auth.users.updated_on</code>.
     */
    public void setUpdatedOn(Timestamp value) {
        set(13, value);
    }

    /**
     * Getter for <code>auth.users.updated_on</code>.
     */
    public Timestamp getUpdatedOn() {
        return (Timestamp) get(13);
    }

    /**
     * Setter for <code>auth.users.updated_by_user</code>.
     */
    public void setUpdatedByUser(String value) {
        set(14, value);
    }

    /**
     * Getter for <code>auth.users.updated_by_user</code>.
     */
    public String getUpdatedByUser() {
        return (String) get(14);
    }

    /**
     * Setter for <code>auth.users.never_expires</code>.
     */
    public void setNeverExpires(Boolean value) {
        set(15, value);
    }

    /**
     * Getter for <code>auth.users.never_expires</code>.
     */
    public Boolean getNeverExpires() {
        return (Boolean) get(15);
    }

    /**
     * Setter for <code>auth.users.reactivated_date</code>.
     */
    public void setReactivatedDate(Timestamp value) {
        set(16, value);
    }

    /**
     * Getter for <code>auth.users.reactivated_date</code>.
     */
    public Timestamp getReactivatedDate() {
        return (Timestamp) get(16);
    }

    /**
     * Setter for <code>auth.users.force_password_change</code>.
     */
    public void setForcePasswordChange(Boolean value) {
        set(17, value);
    }

    /**
     * Getter for <code>auth.users.force_password_change</code>.
     */
    public Boolean getForcePasswordChange() {
        return (Boolean) get(17);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<Integer> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record18 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row18<Integer, String, String, Timestamp, String, String, String, String, Integer, Integer, Timestamp, Timestamp, String, Timestamp, String, Boolean, Timestamp, Boolean> fieldsRow() {
        return (Row18) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row18<Integer, String, String, Timestamp, String, String, String, String, Integer, Integer, Timestamp, Timestamp, String, Timestamp, String, Boolean, Timestamp, Boolean> valuesRow() {
        return (Row18) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Integer> field1() {
        return Users.USERS.ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field2() {
        return Users.USERS.EMAIL;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field3() {
        return Users.USERS.PASSWORD_HASH;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field4() {
        return Users.USERS.PASSWORD_LAST_CHANGED;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field5() {
        return Users.USERS.STATE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field6() {
        return Users.USERS.FIRST_NAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field7() {
        return Users.USERS.LAST_NAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field8() {
        return Users.USERS.COMMENTS;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Integer> field9() {
        return Users.USERS.LOGIN_FAILURES;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Integer> field10() {
        return Users.USERS.LOGIN_COUNT;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field11() {
        return Users.USERS.LAST_LOGIN;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field12() {
        return Users.USERS.CREATED_ON;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field13() {
        return Users.USERS.CREATED_BY_USER;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field14() {
        return Users.USERS.UPDATED_ON;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field15() {
        return Users.USERS.UPDATED_BY_USER;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field16() {
        return Users.USERS.NEVER_EXPIRES;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field17() {
        return Users.USERS.REACTIVATED_DATE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field18() {
        return Users.USERS.FORCE_PASSWORD_CHANGE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer component1() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component2() {
        return getEmail();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component3() {
        return getPasswordHash();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component4() {
        return getPasswordLastChanged();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component5() {
        return getState();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component6() {
        return getFirstName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component7() {
        return getLastName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component8() {
        return getComments();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer component9() {
        return getLoginFailures();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer component10() {
        return getLoginCount();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component11() {
        return getLastLogin();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component12() {
        return getCreatedOn();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component13() {
        return getCreatedByUser();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component14() {
        return getUpdatedOn();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component15() {
        return getUpdatedByUser();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component16() {
        return getNeverExpires();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component17() {
        return getReactivatedDate();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component18() {
        return getForcePasswordChange();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer value1() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value2() {
        return getEmail();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value3() {
        return getPasswordHash();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value4() {
        return getPasswordLastChanged();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value5() {
        return getState();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value6() {
        return getFirstName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value7() {
        return getLastName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value8() {
        return getComments();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer value9() {
        return getLoginFailures();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer value10() {
        return getLoginCount();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value11() {
        return getLastLogin();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value12() {
        return getCreatedOn();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value13() {
        return getCreatedByUser();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value14() {
        return getUpdatedOn();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value15() {
        return getUpdatedByUser();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value16() {
        return getNeverExpires();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value17() {
        return getReactivatedDate();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value18() {
        return getForcePasswordChange();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value1(Integer value) {
        setId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value2(String value) {
        setEmail(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value3(String value) {
        setPasswordHash(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value4(Timestamp value) {
        setPasswordLastChanged(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value5(String value) {
        setState(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value6(String value) {
        setFirstName(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value7(String value) {
        setLastName(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value8(String value) {
        setComments(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value9(Integer value) {
        setLoginFailures(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value10(Integer value) {
        setLoginCount(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value11(Timestamp value) {
        setLastLogin(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value12(Timestamp value) {
        setCreatedOn(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value13(String value) {
        setCreatedByUser(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value14(Timestamp value) {
        setUpdatedOn(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value15(String value) {
        setUpdatedByUser(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value16(Boolean value) {
        setNeverExpires(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value17(Timestamp value) {
        setReactivatedDate(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord value18(Boolean value) {
        setForcePasswordChange(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UsersRecord values(Integer value1, String value2, String value3, Timestamp value4, String value5, String value6, String value7, String value8, Integer value9, Integer value10, Timestamp value11, Timestamp value12, String value13, Timestamp value14, String value15, Boolean value16, Timestamp value17, Boolean value18) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        value11(value11);
        value12(value12);
        value13(value13);
        value14(value14);
        value15(value15);
        value16(value16);
        value17(value17);
        value18(value18);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached UsersRecord
     */
    public UsersRecord() {
        super(Users.USERS);
    }

    /**
     * Create a detached, initialised UsersRecord
     */
    public UsersRecord(Integer id, String email, String passwordHash, Timestamp passwordLastChanged, String state, String firstName, String lastName, String comments, Integer loginFailures, Integer loginCount, Timestamp lastLogin, Timestamp createdOn, String createdByUser, Timestamp updatedOn, String updatedByUser, Boolean neverExpires, Timestamp reactivatedDate, Boolean forcePasswordChange) {
        super(Users.USERS);

        set(0, id);
        set(1, email);
        set(2, passwordHash);
        set(3, passwordLastChanged);
        set(4, state);
        set(5, firstName);
        set(6, lastName);
        set(7, comments);
        set(8, loginFailures);
        set(9, loginCount);
        set(10, lastLogin);
        set(11, createdOn);
        set(12, createdByUser);
        set(13, updatedOn);
        set(14, updatedByUser);
        set(15, neverExpires);
        set(16, reactivatedDate);
        set(17, forcePasswordChange);
    }
}
